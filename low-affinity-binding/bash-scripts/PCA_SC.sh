...
cat >PCA.ptraj<<EOF
	...
	trajin /UB3/pca.nc 1 last 1 parm [ub3]
		#rms to first frame
	rms first :4-101&!@C,CA,N,O,H=
		#construct matrix
	matrix name matc covar :4-101&!@C,CA,N,O,H=
	run
	runanalysis diagmatrix matc out PCA.evecs.dat name diagmodes vecs 5 
	rms first :4-101&!@C,CA,N,O,H=
	projection evecs diagmodes out PCA.project.dat beg 1 end 5 :4-101&!@C,CA,N,O,H=
	run
	quit
EOF
...
cat >PCA.ptraj<<EOF
	...
	runanalysis modes name PCA.evecs.dat trajout PC1.nc trajoutmask :4-101&!@C,CA,N,O,H= pcmin VAR pcmax VAR tmode 1
	runanalysis modes name PCA.evecs.dat trajout PC2.nc trajoutmask :4-101@C,CA,N,O pcmin VAR pcmax VAR tmode 2
EOF
